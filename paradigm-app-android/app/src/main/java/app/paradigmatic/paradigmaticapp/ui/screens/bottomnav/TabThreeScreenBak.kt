...
import app.paradigmatic.paradigmaticapp.data.beacon.IBeaconDeviceItem
                ...
                item {
                    ...
                    Column(
                        ...
                        Button(
                            ...
                            Text(text = if (isScanning) "Stop Scanning" else "Load routes",
                                fontSize = 12.sp,
                                color = Color.White
                            )
                        }
                        ...
                        /* TODO-FIXME-CLEANUP
                        Text(
                            text = ibeaconState.uuid.toString(),
                        ...
                        IBeaconDeviceItem(ibeaconState)
                        */
                    }

                }
                items(
                    items = iBeaconDeviceListState,
                    key = { item -> item.getAddress()}
                ) { ibeaconDevice ->
                    IBeaconDeviceItem(ibeaconDevice)
                }
                /*
                items(bleDeviceListState) { bleDevice ->
                    BLEDeviceItem(navigator, bleDevice)
                }
                */
                if (iBeaconDeviceListState.isEmpty()) {
                    item {
...
fun getTxPowerLevel(scanRecord: ByteArray): Int {
    ...
    val pos: Int = findCodeInBuffer(
        scanRecord,
        BeaconProToolsService.AssignedNumbers.TXPOWER
    )
    return if (pos > 0) {
